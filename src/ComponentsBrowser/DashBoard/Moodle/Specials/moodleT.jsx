...
import MoodlePage1 from './page1';
import MoodlePage2 from './page2';
import MoodleCalendar from './moodlecal';
const MoodleT = ({ setState }) => {
    const date = new Date()
        ...
        <div className='MoodleContainer'>
            <div className='MoodleContent'>
                ...
                {page === 0 ?
                    ...
                    :
                    <>
                        <MoodlePage2 setExtra={setExtra} setContent={setContent} extracontent={extra} classid={classid} classcontent={classcontent} back={back} Clicked={Clicked} />
                        <div style={{ marginRight: '44%', fontSize: '50px', marginTop: '20px', color: 'rgba(0, 204, 255, 0.25)' }}>{extra ? <VscDiffRemoved onClick={e => { setExtra(!extra) }} style={{ cursor: 'pointer', color: '#fdc082' }} /> : <MdPostAdd onClick={e => { setExtra(!extra) }} style={{ cursor: 'pointer', color: '#fdc082' }} />}</div>
                    </>
                }
            </div>
